...
export default function NewEmployeePage() {
  ...
  async function onSubmit(values: FormValues, saveAndNew = false) {
    // strip empty strings to null for optional fields the API expects as None
    const clean: any = { ...values };
    Object.keys(clean).forEach(k => { if (clean[k] === "") clean[k] = null; });
    clean.addresses = (values.addresses || []).map((a: any) => ({ ...a, ...(a.line1 || a.city ? {} : {}) }));
    try {
            ...
            <CardContent>
              <Tabs value={tab} onValueChange={setTab} defaultValue="personal">
                ...
                <TabsContent value="docs">
                  ...
                  />
                  <label
                    htmlFor="new-employee-document-file"
